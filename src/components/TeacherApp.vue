      ...
      <transition name="fade" mode="out-in">
        ...
        <div v-if="currentPage==='dashboard'" key="dashboard">
          ...
          <div v-if="sessionCreated" class="notif-bar notif-green">
            <span>✅</span>
            <span>Session <strong>{{ currentSession.name }}</strong> lancée · Durée : <strong>{{ dureeAffichee }}</strong></span>
          ...
          <div class="content">
            ...
            <div class="stats-row">
              ...
              <div class="stat-card">
                ...
                <div class="stat-value">{{ stats.fcMoyenne }}<span style="font-size:18px;font-weight:400;color:var(--muted)">bpm</span></div>
              ...
              <div class="chart-card">
                ...
                <!-- Zone surcharge -->
                 <rect x="0" y="0" width="500" height="30" fill="rgba(240,83,100,0.06)"/>
                  <line x1="0" y1="30" x2="500" y2="30" stroke="#f05364" stroke-width="1" stroke-dasharray="6,4" opacity="0.4"/>
                 <text x="460" y="22" fill="#f05364" font-size="10" text-anchor="end" font-family="DM Sans">Zone surcharge</text>
                 <!-- Zone remplie dynamique -->
                <path v-if="pathCMGroupe" :d="pathCMGroupeZone" fill="url(#fillGrad)"/>
                <!-- Courbe dynamique -->
                <path v-if="pathCMGroupe" :d="pathCMGroupe" fill="none" stroke="#f5c542" stroke-width="2.5" stroke-linecap="round"/>
                <!-- Message si pas encore de données -->
                <text v-if="!pathCMGroupe" x="250" y="75" text-anchor="middle" font-size="12" fill="#6b6b85" font-family="DM Sans">
                 En attente des données...
                </text>
               <text x="4" y="138" fill="#6b6b85" font-size="10" font-family="DM Sans">0:00</text>
               <text x="470" y="138" fill="#6b6b85" font-size="10" font-family="DM Sans">{{ currentSession.elapsed }}</text>
               </svg>
              </div>
              ...
              <div class="chart-card">
                ...
                <div class="student-list">
                  <div v-for="s in students" :key="s.name" class="student-row">
                    ...
                    <span style="font-size:10px;color:var(--muted);width:30px;text-align:right;text-transform:uppercase;">{{ s.levelLabel }}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="form-card">
                ...
                <div class="form-group">
                  ...
                  <div class="duration-row">
                    ...
                    <select v-if="sessionForm.durationType==='preset'" class="form-select" v-model="sessionForm.duration">
                    ...
                    <div v-else class="custom-duration">
                      <input class="form-input" type="number" v-model.number="sessionForm.customHours" min="0" max="5" placeholder="0" style="width:72px;text-align:center"/>
                      <span class="duration-unit">h</span>
                      <input class="form-input" type="number" v-model.number="sessionForm.customMinutes" min="0" max="59" placeholder="00" style="width:72px;text-align:center"/>
          ...
          <div class="content">
            <div class="three-col">
              ...
              <div class="stat-card">
                ...
                <div class="tlx-bar-wrap">
                  ...
                  <div class="tlx-track"><div class="tlx-fill" style="background:var(--purple)" :style="{width: tlx.debut+'%'}"></div></div>
                  <div class="tlx-score">{{ tlx.debut }} / 100</div>
                </div>
                <div class="tlx-bar-wrap" style="margin-top:14px">
                  <div class="tlx-label">Fin de session</div>
                  <div class="tlx-track"><div class="tlx-fill" style="background:var(--yellow)" :style="{width: tlx.fin+'%'}"></div></div>
            ...
            <div class="chart-card">
              ...
              <table class="data-table">
                <thead>
                  <tr>
                    <th>Étudiant</th><th>FC Moy.</th><th>HRV</th><th>NASA-TLX</th><th>Score Global</th><th>Niveau</th><th>Action</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <div class="content">
            <div class="hist-card">
              <table class="hist-table">
                <thead>
                  <tr>
                    <th>Date</th><th>Cours</th><th>Groupe</th><th>Étudiants</th><th>CM Groupe</th><th>Niveau</th><th></th>
                  </tr>
                </thead>
                ...
                <tbody>
                  <tr v-for="h in history" :key="h.date">
                    ...
                    <td><button class="btn btn-ghost" style="padding:5px 14px;font-size:12px" @click="exportPDF('rapport', h)">Rapport</button></td>
                  </tr>
                </tbody>
              </table>
            </div>
...
.logo { display: flex; align-items: center; gap: 10px; padding: 0 20px 24px; font-family: 'Syne', sans-serif; font-size: 18px; font-weight: 700; }
.logo-icon { width: 34px; height: 34px; border-radius: 10px; background: linear-gradient(135deg, #3b1f8c, #7c5cfc); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.nav-section { padding: 0 12px; }
.nav-label { font-size: 10px; letter-spacing: .1em; color: var(--muted); padding: 0 8px 8px; text-transform: uppercase; }
.nav-item { display: flex; align-items: center; gap: 10px; padding: 9px 12px; border-radius: 8px; cursor: pointer; color: var(--muted); transition: all .15s; margin-bottom: 2px; font-size: 13.5px; }
.nav-item:hover { background: var(--card); color: var(--text); }
.nav-item.active { background: rgba(124,92,252,.18); color: var(--purple-light); }
...
.avatar { width: 32px; height: 32px; border-radius: 50%; background: linear-gradient(135deg, #5a3fbf, #8c55f5); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: #fff; flex-shrink: 0; }
.user-info { flex: 1; }
...
.notif-bar { margin: 16px 36px 0; border-radius: 10px; padding: 13px 18px; display: flex; align-items: center; gap: 12px; font-size: 13px; animation: slideDown .3s ease; }
.notif-green { background: rgba(61,214,140,.1); border: 1px solid rgba(61,214,140,.3); color: var(--green); }
.notif-purple { background: rgba(124,92,252,.12); border: 1px solid rgba(124,92,252,.4); color: var(--purple-light); }
.notif-bar strong { color: #fff; }
.notif-close { margin-left: auto; background: none; border: none; color: var(--muted); cursor: pointer; font-size: 14px; padding: 0 4px; }
.notif-close:hover { color: var(--text); }
@keyframes slideDown { from{opacity:0;transform:translateY(-8px)} to{opacity:1;transform:translateY(0)} }

.page-header { padding: 28px 36px 0; display: flex; align-items: flex-start; justify-content: space-between; }
.page-title { font-family: 'Syne', sans-serif; font-size: 28px; font-weight: 800; }
.page-title span { color: var(--muted); font-weight: 400; font-size: 18px; margin-left: 10px; }
.page-sub { font-size: 12.5px; color: var(--muted); margin-top: 4px; display: flex; align-items: center; gap: 6px; }
.live-dot { width: 7px; height: 7px; border-radius: 50%; background: var(--green); animation: pulse 1.6s infinite; flex-shrink: 0; }
.live-dot.stopped { background: var(--muted); animation: none; }
@keyframes pulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.5;transform:scale(1.3)} }

.content { padding: 24px 36px; flex: 1; }
.stats-row { display: grid; grid-template-columns: repeat(4,1fr); gap: 16px; margin-bottom: 20px; }
.stat-card { background: var(--card); border: 1px solid var(--border); border-radius: 14px; padding: 20px 22px; }
.stat-label { font-size: 10.5px; letter-spacing: .08em; color: var(--muted); text-transform: uppercase; margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between; }
.stat-value { font-family: 'Syne',sans-serif; font-size: 38px; font-weight: 800; line-height: 1; }
.stat-sub { font-size: 12px; color: var(--muted); margin-top: 6px; }

.badge { font-size: 10px; font-weight: 700; letter-spacing: .06em; text-transform: uppercase; padding: 3px 8px; border-radius: 6px; }
.badge-mod  { background: rgba(245,197,66,.15); color: var(--yellow); }
.badge-elev { background: rgba(240,83,100,.15); color: var(--red); }
.badge-faib { background: rgba(61,214,140,.15); color: var(--green); }
...
.card-title { font-size: 10.5px; letter-spacing: .09em; color: var(--muted); text-transform: uppercase; }
.live-badge { font-size: 10px; color: var(--blue); display: flex; align-items: center; gap: 5px; }
.live-badge::before { content:''; width:6px; height:6px; border-radius:50%; background:var(--blue); display:inline-block; animation:pulse 1.6s infinite; }
...
.student-name { width: 72px; font-size: 12.5px; color: var(--text); flex-shrink: 0; }
.bar-track { flex: 1; height: 8px; background: var(--bg); border-radius: 4px; overflow: hidden; }
.bar-fill { height: 100%; border-radius: 4px; transition: width .6s ease; }
.bar-mod  { background: linear-gradient(90deg,#6060f0,#a07cf8); }
.bar-elev { background: linear-gradient(90deg,#f05364,#f5834a); }
.bar-faib { background: linear-gradient(90deg,#3dd68c,#4db8ff); }
.student-score { width: 28px; text-align: right; font-family:'Syne',sans-serif; font-weight:700; font-size:14px; }
.score-mod  { color: var(--yellow); }
.score-elev { color: var(--red); }
.score-faib { color: var(--green); }
.level-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }

.alert-bar { background: rgba(245,197,66,.08); border: 1px solid rgba(245,197,66,.25); border-radius: 10px; padding: 12px 18px; display: flex; align-items: flex-start; gap: 12px; font-size: 12.5px; }
.alert-icon { font-size: 16px; flex-shrink: 0; margin-top: 1px; }
.alert-text strong { color: var(--yellow); }

.btn { padding: 9px 18px; border-radius: 8px; cursor: pointer; font-family:'DM Sans',sans-serif; font-size: 13px; font-weight: 600; border: none; transition: all .15s; }
...
.form-card h2 { font-family:'Syne',sans-serif; font-size: 17px; font-weight: 700; margin-bottom: 22px; }
.form-group { margin-bottom: 18px; }
.form-label { font-size: 10.5px; letter-spacing: .09em; text-transform: uppercase; color: var(--muted); margin-bottom: 7px; display: block; }
.form-input, .form-select { width: 100%; background: var(--bg); border: 1px solid var(--border); border-radius: 8px; padding: 10px 14px; color: var(--text); font-family:'DM Sans',sans-serif; font-size: 13.5px; outline: none; transition: border-color .15s; }
.form-input:focus, .form-select:focus { border-color: var(--purple); }
.form-select { appearance: none; cursor: pointer; }
.form-actions { display: flex; gap: 12px; margin-top: 24px; }
.duration-row { display: flex; flex-direction: column; gap: 10px; }
.custom-duration { display: flex; align-items: center; gap: 8px; }
.duration-unit { color: var(--muted); font-size: 13px; font-weight: 600; }

.side-card { background: var(--card); border: 1px solid var(--border); border-radius: 14px; padding: 22px; margin-bottom: 16px; }
.side-title { font-size: 10.5px; letter-spacing: .09em; text-transform: uppercase; color: var(--muted); margin-bottom: 14px; }
.student-item { display: flex; align-items: center; gap: 10px; padding: 7px 0; border-bottom: 1px solid var(--border); }
.student-item:last-child { border-bottom: none; }
.s-avatar { width: 28px; height: 28px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; flex-shrink: 0; }
.fitbit-badge { font-size: 10px; padding: 3px 8px; border-radius: 5px; background: rgba(77,184,255,.12); color: var(--blue); font-weight: 600; margin-left: auto; }
.disconnected { font-size: 11px; color: var(--muted); margin-left: auto; }
.recap-row { display: flex; justify-content: space-between; align-items: center; padding: 6px 0; border-bottom: 1px solid var(--border); font-size: 12.5px; }
.recap-row:last-child { border-bottom: none; }
.recap-key { color: var(--muted); }
.recap-val { font-weight: 600; }
.api-badge { font-size: 10px; padding: 3px 8px; border-radius: 5px; background: rgba(61,214,140,.12); color: var(--green); font-weight: 700; }
.api-badge::before { content:'● '; font-size: 8px; }

.three-col { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 16px; margin-bottom: 20px; }
.bar-chart-group { display: flex; align-items: flex-end; gap: 12px; height: 100px; padding-top: 10px; }
.bar-group-item { display: flex; flex-direction: column; align-items: center; gap: 6px; flex: 1; }
.bar-group-rect { width: 100%; border-radius: 6px 6px 0 0; }
.bar-group-label { font-family:'Syne',sans-serif; font-size: 16px; font-weight: 800; }
.tlx-bar-wrap { margin-bottom: 12px; }
.tlx-label { font-size: 11.5px; color: var(--muted); margin-bottom: 4px; }
.tlx-track { height: 7px; background: var(--bg); border-radius: 4px; overflow: hidden; margin-bottom: 3px; }
.tlx-fill { height: 100%; border-radius: 4px; }
.tlx-score { font-family:'Syne',sans-serif; font-weight: 700; font-size: 14px; }
.tlx-delta { font-size: 11px; color: var(--green); }
...
.gauge-score { font-family:'Syne',sans-serif; font-size: 40px; font-weight: 800; margin-top: -10px; }
.gauge-sub { font-size: 11px; }

.data-table { width: 100%; border-collapse: collapse; }
.data-table th { font-size: 10px; letter-spacing: .09em; text-transform: uppercase; color: var(--muted); padding: 0 0 12px; text-align: left; border-bottom: 1px solid var(--border); }
.data-table td { padding: 14px 0; border-bottom: 1px solid rgba(255,255,255,.04); font-size: 13px; }
.data-table tr:last-child td { border-bottom: none; }
.col-score { font-family:'Syne',sans-serif; font-weight: 800; font-size: 16px; }

.hist-card { background: var(--card); border: 1px solid var(--border); border-radius: 14px; overflow: hidden; }
.hist-table { width: 100%; border-collapse: collapse; }
.hist-table th { font-size: 10px; letter-spacing: .09em; text-transform: uppercase; color: var(--muted); padding: 16px 22px; text-align: left; border-bottom: 1px solid var(--border); }
.hist-table td { padding: 16px 22px; border-bottom: 1px solid rgba(255,255,255,.04); font-size: 13px; }
